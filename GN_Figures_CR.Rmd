...
out <- readRDS(file="./models/zinb_gn_model_brms.rds" )


# Convert mcmc draws to matrix
posterior <- as.matrix(out)

posterior_df <- as.data.table(posterior)[ ,c(1:13),] #take only the covariate columns
```
...
BACI_Figure
ggsave("./zinb_figures/figure_2_GN.jpeg", height = 10, width = 10)
```
...
post_dist_blue2
ggsave("./zinb_figures/figure_3_GN.jpeg", height = 10, width = 10)
```
...
stocked_fry <- exp(posterior_df$b_Intercept + posterior_df$b_fry1) 

stocked_fgl <- exp(posterior_df$b_Intercept + posterior_df$b_fingerling1)
...
Stocking_Figure
ggsave("./zinb_figures/figure_4_GN.jpeg", height = 10, width = 10)
```
...
Fig_5 <- plot_grid(gdd_LL_Plot, gdd_Plot_sum,  area_Plot,
          ...
          label_size = 14)
Fig_5
ggsave("./zinb_figures/figure_5_gn.jpeg", height = 10, width = 10)
```
