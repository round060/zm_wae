...
dow_year <- new_surveys_secchi %>% distinct(lake_id, year)
...
surveys_no_temp <- merge(new_surveys_secchi, dow_nhdhr, by = c("lake_id"), all.x = T)
...
model_surveys_ef <- model_surveys %>% 
  merge(y = average_temps, by = "site_id") %>%
  merge(y = secchi_average, by.y = "nhdhr.id", by.x = "site_id", all.x = T)


yoy_ef <- model_surveys_ef %>%
  select(!c(site_id, nhdhr.id, lagos.id, acres, shore_mi, infested))
