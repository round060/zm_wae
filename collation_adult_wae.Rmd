...
```{r}
# From a beta version of MidFish: A Large-Scale Aggregation of State Fisheries Data from the Midwestern United States from 1940 - 2023 - ScienceBase-Catalog
aged <- open_dataset("./data/age_assigned_data/mn_halk_aged_data/part-0.parquet") %>% collect() 

# From Mn geospatial commons
# Minnesota Department of Natural Resources (MNDNR). 2021. Lake Bathymetric Outlines, Contours, and DEM.
...
all_aged <- adult_walleye_cpue %>% filter(all_aged == 1) %>% mutate(age_cat = "all aged")
# 2638
...
walleye_survey_cpue <- rbind(all_aged, not_all_aged_lake_alk, not_all_aged_year_alk) %>% 
  mutate(survey_id = str_c(lake.id, year, total.effort.1, sampling.method))
...
# From https://www.dnr.state.mn.us/invasives/ais/infested.html
...
# Add in GDD
```{r}
# From Corson-Dosch -  https://doi.org/10.5066/P9EQQER7
...
length(unique(year_3_fish$lake.id))
