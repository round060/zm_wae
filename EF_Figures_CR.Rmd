...
```{r, message=FALSE}
EF_Final <- read_csv("./cleaned_data/yoy_catch_data.csv") %>%
...
EF_Lakes_ZM <- ggplot(data = MN_Counties, mapping = aes(x = long, y = lat, group=group))+
  ...
  geom_point(data=EF_Final, aes(x= x, y = y, group=25, color=as.factor(ZM_LAKE),
...
post_dist_blue2 <- mcmc_areas(
    posterior,
    pars = c("b_trt", "b_post","b_stocked","b_post_trt","b_lake_area","b_gdd","b_survey_gdd"),
  ...
  scale_y_discrete(limit = c("b_trt", "b_post", "b_post_trt", "b_lake_area", "b_stocked","b_survey_gdd", "b_gdd"),
...
#
ggsave("./figures/figure_3.jpeg", height = 10, width = 10)
```
...
Stocking_Figure
ggsave("./figures/figure_4.jpeg", height = 10, width = 10)
```
...
annual_gdd <- as.numeric(scale(dat$gdd_wtr_5c)) # GDD up to day of sampling
post <- dat$post
...
b_gdd_sum <- posterior_df$b_survey_gdd
b_0 <- posterior_df$b_0

sum <- dat$anomaly_gdd_5 
...
```{r}
Fig_5 <- plot_grid(gdd_LL_Plot, GDD_Plot,  area_Plot,
          labels = c("A", "B", "C"),
          label_size = 14)
Fig_5
ggsave("./figures/figure_5.jpeg", height = 10, width = 10)
